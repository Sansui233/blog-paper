...
export async function buildMemoSearchIndex(memosData: Memo[]) {
  ...
  const sortedFiles = [...memosData].sort((a, b) =>
  ...
  console.log(
    `🔍 [searchindex.ts] ${index.length} memos indexed (${recentMemos.length} recent + ${taggedMemos.length} tagged, deduplicated)`,
  );
}
